-- ============================================================================
-- Sello — Market-readiness: annual pricing + GST fields + UPI groundwork
-- Run AFTER 0008.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1) Reprice plans to annual (₹/year), aligned to the Indian market
-- ---------------------------------------------------------------------------
alter table public.plans rename column price_monthly to price_yearly;

update public.plans set price_yearly = case id
  when 'free'     then 0
  when 'starter'  then 1999
  when 'pro'      then 3999
  when 'business' then 7999
...
alter table public.sale_items add column if not exists gst_rate numeric(5, 2) not null default 0;

-- ---------------------------------------------------------------------------
-- 3) create_sale() — now snapshots each line's GST rate from the product
-- ---------------------------------------------------------------------------
